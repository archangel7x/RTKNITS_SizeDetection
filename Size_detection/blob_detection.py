...
def duplicate_check(path):
    if os.path.isfile(path):
        choice = str(input(f"[WARNING]:filename:{[path]} already exists!, are you sure you wish to continue? [y/n]:"))
        if choice == 'y':
            cond =False 
            return cond 
...
def main(path,saving_directory):
    ...
    while cond == True:
        ...
        filename = f'output_image{ident}.jpg'
        joined_path = os.path.join(saving_directory, filename)
        cond = duplicate_check(joined_path)
   ...
   # cv.namedWindow('Keypoints', cv.WINDOW_NORMAL)  # Create a resizable window
    ...
    d.draw(keypoints_coordinates, size, im_with_keypoints, thickness2,camera_height,font,org,fontScale,ident,saving_directory)  
