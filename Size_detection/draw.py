...
def draw(keypoints_coordinates, size, image, thickness,camera_height,font,org,fontScale,ident,saving_directory,after_wash):
    top_right = []
    top_left = []
    bottom_right = []
    bottom_left = []
...
# Define the positions for putting text next to each line
   ...
   # Draw text next to each line
    ...
    s.save_dataframe(ident, distance_top, distance_right, distance_bottom, distance_left,after_wash ,csv_file='detection.csv')
